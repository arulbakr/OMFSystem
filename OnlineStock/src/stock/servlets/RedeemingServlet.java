...
/**
 * Servlet implementation class RedeemingServlet
 */
@WebServlet("/RedeemingServlet")
public class RedeemingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Executed doGet of RedeemingServlet");
		...
		if(uSession != null)
		{
			...
			try {
				// Loading funds DD
				List<FundTypeEntity> fundTypes = txnManager.getFundTypes(uSession.getUserId());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Executed doPost of RedeemingServlet");
		int result = 0;
		HttpSession session = request.getSession(false);
		UserSessionEntity uSession = session != null ? (UserSessionEntity) session.getAttribute("UserSession") : null;
		if(uSession != null)
		{
			...
			try {
				result = txnManager.redeemUserFund(ufEntity);
				if(result > 0){
					request.setAttribute(Constants.SUCCESS_MESSAGE, Messages.REDEEM_SUCCESS);
				} else if(result == -1) {
					request.setAttribute(Constants.ERROR_MESSAGE, Messages.REDEEM_FAILURE);
				} else {
